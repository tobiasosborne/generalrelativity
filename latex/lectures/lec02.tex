...
\begin{tikzpicture}[scale=0.9]
  ...
  % Lab frame (x) — level sets from dipole tidal field
  % Data: generated by scripts/sim_lec02_coords.jl
  \begin{scope}[shift={(5,0)}]
    ...
    % Coordinate level sets (computed, not splines)
    \foreach \i in {1,...,4} {
      \draw[thin, spacecadet!20] plot file {data/lec02_coord_h\i.dat};
      \draw[thin, spacecadet!20] plot file {data/lec02_coord_v\i.dat};
    }
  \end{scope}
\end{tikzpicture}
\end{center}
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_lec02_geodesic_bump}

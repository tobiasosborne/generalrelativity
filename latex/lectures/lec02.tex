...
\begin{figure}[htbp]
  ...
  \caption{Geodesics on a Gaussian-bump surface
    $z = e^{-(x^2+y^2)/2}$.
    Top: 3D view showing geodesics (coloured curves) bending
    over the bump.
    Bottom: top-down view comparing geodesics (solid) with
    Euclidean straight lines (dashed).}\label{fig:geodesic-bump}
\end{figure}

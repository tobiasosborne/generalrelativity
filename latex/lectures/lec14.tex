...
itself can change with time.  Just as the electromagnetic field in
Maxwell's theory supports wave-like solutions, the dynamical nature
of the gravitational field opens the possibility of
\textbf{gravitational radiation}---wave-like solutions of the
linearised vacuum Einstein equations that propagate at the speed
of light.  We can already study gravitational radiation at the
linearised level of approximation; the full nonlinear theory adds
corrections but the essential physics is captured here.
...
radiation gauge).  The way we find waves is the way we \emph{always}
find waves: we make a plane-wave ansatz and substitute it into the
field equations to see what conditions emerge.  Specifically:
...
\begin{keyresult}[Two polarizations]
  ...
  Two is bigger than zero!  It is a striking prediction of
  Einstein's theory that the gravitational field has dynamical,
  wave-like solutions---a feature entirely absent from Newtonian
  gravity.
\end{keyresult}
...
The degree-of-freedom count is dimension-dependent, and it is
particularly interesting---and highly relevant for current
investigations into quantum gravity---to ask whether gravitational
waves exist in lower dimensions.  In $(2{+}1)$-dimensional
spacetime:
\begin{itemize}
...
We could, in fact, have guessed this result without any
calculation.  In $(2{+}1)$ dimensions, the Riemann tensor is
completely determined by the Ricci tensor and scalar:
...
identically in three dimensions.  To see why, recall that the
Riemann tensor may be decomposed into a part determined by~$R_{ab}$
and~$R$, plus a completely trace-free remainder---the Weyl tensor.
In $(2{+}1)$ dimensions a component count shows that the Weyl tensor
has zero independent components, so it must vanish.

\begin{exercise}\label{ex:weyl-2+1}
  Verify the vanishing of the Weyl tensor in $(2{+}1)$ dimensions by
  counting the independent components of the Riemann tensor (using
  its symmetries) and comparing with the number of independent
...
It is impossible to discuss gravitational radiation without
mentioning one of the most dramatic discoveries of modern physics.
...
\begin{historical}[LIGO and the first direct detection]
  ...
  one-thousandth the diameter of a proton.  A significant portion
  of the data analysis was carried out in Hannover, Germany, at the
  Albert Einstein Institute.  The 2017 Nobel Prize in Physics was
  awarded to Rainer Weiss, Barry Barish, and Kip Thorne for this
  discovery.
\end{historical}
...
We now turn from perturbative solutions to a far more ambitious
programme: finding \emph{exact} solutions of Einstein's field
equations that describe the large-scale structure of the entire
universe.  Our goal is to be as bold as possible---to extend the
domain of applicability of Einstein's equations to the cosmos as a
whole, to make a model of how matter is distributed throughout the
universe, and then to solve the field equations.

Obviously we cannot do this for an arbitrary matter distribution:
Einstein's equations are far too complicated.  Instead we impose
massive simplifications in the form of symmetry assumptions.
The key observational fact is that, on sufficiently large scales
(hundreds of megaparsecs), the distribution of matter in the
universe appears remarkably uniform: whether a star is here or
there does not matter if one averages on galactic length scales.
There is no preferred location and no preferred direction.  We
formalise these observations as the assumptions of
\textbf{homogeneity} and \textbf{isotropy}.
...
Before defining isotropy, we note an important subtlety.  Imagine
the universe is filled with an isotropic distribution of matter and
there is at least one observer at a point~$p$ who sees the universe
as isotropic.  Can there be another observer at the same point who
also sees isotropy?  The answer is \emph{no}: any boosted observer
perceives the background matter distribution as distorted---thanks
to length contraction, the stress-energy appears concentrated in the
directions perpendicular to the boost, destroying isotropy.
Therefore, at each point there is \emph{at most one} observer who
perceives the universe as isotropic, and our definition must refer
to this preferred family of observers.

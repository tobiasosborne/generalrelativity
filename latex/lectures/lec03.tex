...
relativity---indeed of much of physics---that spacetime is a
continuum: to characterise an event we need four numbers (one
temporal, three spatial).  In special relativity this continuum is
the familiar flat $\Rn{4}$, but in general relativity the geometry
itself is dynamical---it changes in response to the distribution of
matter, energy, and stress throughout the universe.
...
whereas in physics we want to make sharp, falsifiable predictions.
One approach would be to model curved continua as surfaces embedded
in a higher-dimensional ambient space~$\Rn{m}$.  This works, but it
raises an uncomfortable question: do these extra dimensions exist?
We sidestep the issue entirely by studying curved continua
\emph{intrinsically}, without reference to any ambient
space---and this leads us to the theory of \emph{manifolds}.
...
\begin{intuition}[Why manifolds?]
  ...
  The intrinsic approach requires more formalism upfront, but it is
  ultimately more natural and more transferable to other areas of
  theoretical physics.
\end{intuition}
...
We work in $n$ dimensions throughout.  One might ask: why bother
with arbitrary~$n$ when spacetime is four-dimensional?  There are
several good reasons.  First, it costs almost no extra effort and
yields considerable generality for free.  Second, we often wish to
restrict attention to dynamics on a lower-dimensional hypersurface
(a particle constrained to the surface of a sphere, for instance).
Third---and more speculatively---there are indications from
quantum gravity (the holographic principle, for example) that the
effective dimension of spacetime may itself be emergent.  Finally,
in a quantum theory of gravity one might need to superpose spacetimes
of different dimensions, so it is wise not to hard-code $n=4$.
...
\begin{itemize}
  ...
  \item $C^k$ denotes the set of $k$-times continuously differentiable
    functions (from $\Rn{n}$ to $\R$, or between open subsets thereof):
    all partial derivatives of order $\le k$ exist and are continuous.
...
\begin{definition}[Manifold]\label{def:manifold}
  ...
  \begin{enumerate}
    \item[\textup{(0)}] $\M$ is a topological space (Hausdorff and
      second-countable).\footnote{The Hausdorff and second-countability
      conditions exclude pathological examples; together they imply
      paracompactness and hence the existence of partitions of unity.}
    ...
    \item[\textup{(2)}] For each $\alpha$ there is a
      \textbf{homeomorphism}
      \[
      ...
      \input{figures/fig_lec03_chart_single}

      One should think of each pair $(O_\alpha,\psi_\alpha)$ as
      identifying a piece of the manifold with a piece of
      Euclidean space.  A manifold is thus \emph{built up---sewn
      together---from patches of~$\Rn{n}$}.
    ...
    \item[\textup{(3)}] If any two sets $O_\alpha$ and $O_\beta$
      ...
      is $C^\infty$.  This condition governs the \emph{gluing}:
      wherever two patches overlap, the two coordinate
      descriptions must be smoothly related.
  \end{enumerate}
\end{definition}

\input{figures/fig_lec03_chart_overlap}

%%WEB-WIDGET:two-charts
...
On the overlap $\{(y,z)\in\Rn{2} \mid y<0,\; y^2+z^2<1\}$, the
...
\input{figures/fig_lec03_sphere}
...
\begin{intuition}[Do we need manifolds?]
  For many basic solutions of Einstein's field equations the
  underlying manifold is simply~$\Rn{4}$---only the geometry
  (the metric) varies.  So one might wonder whether the full
  manifold machinery is really necessary.
  The answer is \emph{yes}: as soon as one considers cosmological
  spacetimes (which may be compact, like $S^3\times\R$) or black
  hole spacetimes (which have non-trivial causal structure and
  topology), more general manifolds become unavoidable.
  In quantum gravity one may even wish to sum over manifolds of
  different topologies, making this generality essential.
\end{intuition}
...
\input{figures/fig_lec03_smoothmap}
...
no natural additive structure.  This is an instance of a general
phenomenon: when one generalises a definition so that it applies to
more examples, some of the structure enjoyed by the original
example is inevitably lost.  Euclidean space is simultaneously a
manifold, a vector space, and a group; a generic manifold is none
of the latter two.

\begin{example}[$S^2$ is not a vector space]
  The 2-sphere $S^2$ is a manifold, but it is \emph{not} a vector
  space: there is no natural way to ``add'' two points on $S^2$
  and obtain another point on~$S^2$.
\end{example}
...
motion for test particles.  The key observation is that even
though~$S^2$ is not globally a vector space, at each point~$p$
there is still a natural \emph{local} vector space---the tangent
plane---and this idea generalises to arbitrary manifolds.
...
\input{figures/fig_lec03_tangent_plane}

But we want an \emph{intrinsic} definition, independent of any
embedding.

\begin{keyresult}[Intrinsic tangent vectors]
  The picture above relies on the embedding $S^2\subset\Rn{3}$,
  which we have declared off-limits.  The solution is a pleasant
  roundabout: one first observes that there is a one-to-one
  correspondence between tangent vectors and \emph{directional
  derivatives} of smooth functions, and then notes that directional
  derivatives can be defined using only the manifold
  structure---no embedding required.  We therefore \emph{define}

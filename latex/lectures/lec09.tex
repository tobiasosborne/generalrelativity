...
\input{figures/fig_lec09_sphere_holonomy}

\noindent
The figure above is the canonical illustration: the initial vector
$w_0 = (1,0,0)$ at the north pole~$p$ is parallel-transported to
$q=(1,0,0)$ along two paths.  Along the meridian~$\gamma_1$ (which is
itself a geodesic and to which $w_0$ is initially tangent) the vector
ends pointing south, $w_{\gamma_1}\!(q) = (0,0,-1)$.  Along
$\gamma_2$ (a meridian to $(0,1,0)$ followed by an arc of the equator)
the vector ends pointing along the equator,
$w_{\gamma_2}\!(q) = (0,-1,0)$.  The two end vectors are perpendicular
--- the angle between them equals the area $\pi/2$ of the spherical
triangle on the unit sphere, exactly as predicted by the

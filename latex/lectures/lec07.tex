...
Recall our earlier discussion of parallel transport: given a curve
on~$\M$, we want a rule that ``carries'' a tensor from one point
to a nearby point along the curve.  One should expect a close
correspondence between such parallel-transport operations and
derivative-type operators.  Indeed, an infinitesimal parallel
transporter is precisely a covariant derivative: the derivative
$\covd_X Y$ measures how a vector field~$Y$ fails to be
``parallel'' as one moves in the direction~$X$.  Our strategy in
this lecture is to abstractly and intrinsically characterise those
properties that any reasonable covariant derivative operator
should possess, and then to explore what freedom remains in
choosing such an operator.
...
\begin{remark}
  We have now assembled a ``wish list'' of seven properties
  (axioms~(0)--(5)) that we would like any covariant derivative
  operator to satisfy.  There is some redundancy among
  these axioms---we have not aimed for the most economical
  characterisation, but rather for a complete catalogue of
  the properties we will need.  Of course, it is all very well to
  write down such a list; the question is whether anything
  \emph{exists} that satisfies it, and if so, how much freedom
  remains.  We address existence and non-uniqueness in
  \S\ref{sec:existence-nonuniqueness} below.
\end{remark}
...
Having written down our list of axioms, we must verify that
something actually satisfies them---otherwise the theory would
be vacuous.  It turns out that there are \emph{many} derivative
operators satisfying axioms~(0)--(5).  We first exhibit the
simplest one, then characterise the freedom in choosing~$\covd$.
...
\noindent
The chart-$\alpha$ derivative differentiates the
\emph{components} of a tensor field, but the basis itself varies
from point to point.  The figure below illustrates this on the
recurring example surface $\M = \{z = h(x,y)\}$: the coordinate
basis $\partial/\partial x^\mu = \partial_\mu\Phi$ is sketched at
four points along a curve.  Although each pair has the same chart
components $(1,0)$ and $(0,1)$ in $\Rn{2}$, the lifted vectors on
$\M$ tilt with the surface.  The Christoffel symbols introduced
below in \S\ref{sec:christoffel} encode this twist.

\input{figures/fig_lec07_chart_basis}
...
\begin{remark}
  This result is rather striking.  A single derivative operator
  $\covd_a$ applied to a covector field~$\omega_b$ certainly
  depends on the behaviour of~$\omega_b$ in a neighbourhood
  of~$p$ (it involves first-order information, just as an
  ordinary derivative does).  Yet the \emph{difference} of two
  derivative operators applied to~$\omega_b$ depends only on
  $\omega_b|_p$ itself---all the ``derivative-like'' information
  cancels.  In other words, the difference
  $\tilde\covd_a - \covd_a$ is not really a differential operator
  at all: it is a purely algebraic, pointwise linear map on
  tensors.
\end{remark}
...
\input{figures/fig_lec07_difference}

\noindent
The figure illustrates the algebraic nature of $C^c{}_{ab}$ on
the recurring example manifold (Lecture~\ref{sec:manifolds}).  The vector
field $W$ has chart-$\alpha$ components $W^u\equiv 1$, $W^v\equiv
0$, so it is constant in chart $\alpha$ and $\partial_a W^b = 0$
identically.  The Levi-Civita connection
(\S\ref{sec:levi-civita} below) of the induced metric gives a
nonzero derivative $\covd_X W$ at the focal point~$p$;
the entire vector shown is the $C$-tensor contribution
$C^\sigma{}_{a\nu}\,X^a W^\nu$, which depends only on the value
of~$W$ at~$p$.
...
\]
Rearranging~\eqref{eq:C-tensor-covector} gives
...
\[
  ...
  + \omega_b\,(\tilde\covd_a - \covd_a)t^b = 0\,.
\]
Relabelling the dummy index $c\leftrightarrow b$ in the first term
and requiring this to hold for all~$\omega_b$, we obtain
$(\tilde\covd_a - \covd_a)t^b = -C^b{}_{ac}\,t^c$, i.e.
...
\medskip
We have seen that the set of torsion-free derivative operators on
a manifold forms an affine space, parameterised by a symmetric
tensor field~$C^c{}_{ab}$.  Without additional structure, there
is no preferred choice of connection.  In the next lecture we
will see that the data of a \emph{metric}~$g_{ab}$ on~$\M$ is
enough to uniquely single out one particular derivative
operator---the \textbf{Levi-Civita connection}---by imposing
the additional requirement $\covd_a g_{bc} = 0$ (metric
compatibility).  That connection will play a central role in the

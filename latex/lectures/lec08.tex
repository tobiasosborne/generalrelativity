...
(or tensor) along a curve ``without rotating it.''

The central goal is to \emph{engineer} a device: given a
derivative operator~$\covd_a$, a curve~$C$ from~$p$ to~$q$, and
a vector $v^a \in V_p$, we want to produce a corresponding
vector in~$V_q$.  That is, we want to build a linear map
$U_C\colon V_p \to V_q$ --- a \textbf{parallel transporter} ---
from the data of the connection alone.  When a metric is present,
requiring that this transporter preserves inner products singles
out a unique derivative operator: the \emph{Levi-Civita
connection}.  We derive its explicit form in coordinates (the
Christoffel symbols) and conclude with the geodesic equation.
...
\input{figures/fig_lec08_parallel_transport}
...
\noindent
The transporter~$U_C$ depends on which connection $\covd_a$ we
choose.  The figure below contrasts two transports of the same
initial vector along the same curve: the chart-$\alpha$ partial
derivative $\partial_a$ (which trivially keeps chart components
constant) and the Levi-Civita connection of the surface metric.
The two prescriptions agree at $p$ and diverge along~$C$ ---
``parallel'' is not absolute, it is a choice of $\covd_a$.

\input{figures/fig_lec08_naive_vs_lc}

% ──────────────────────────────────────────────────────────────
\subsection{The Levi-Civita connection}%
\label{sec:levi-civita}

So far, there is no preferred derivative operator on a bare
manifold---we have an infinite-dimensional family to choose from.
If the manifold $\M$ is additionally equipped with a
metric~$g_{ab}$, however, we can make a \emph{canonical} choice
of derivative operator~$\covd_a$.  The guiding physical
requirement is that parallel transport should preserve lengths
and angles: what kind of ``parallel'' transport would it be if
vectors changed their length or the angle between them varied as
we slid them along a curve?
...
\begin{remark}
  The result is remarkable: the single condition
  $\covd_a g_{bc} = 0$ is neither too strong (it does not
  eliminate all derivative operators) nor too weak (it does not
  leave any freedom).  It is ``just right''---the metric uniquely
  determines the connection.  From now on, whenever a metric
  $g_{ab}$ is given, we choose $\covd_a$ to be the unique
  Levi-Civita connection determined by~$g_{ab}$.
\end{remark}
...
\label{ex:R2-polar}
  ...
  \]
  The factor $r^2$ in front of $\dd\varphi\tp\dd\varphi$ has a
  simple geometric meaning: a small angular displacement
  $\dd\varphi$ produces a larger arc-length displacement the
  further one is from the origin, so the metric must weight the
  angular direction by~$r$.
...
\input{figures/fig_lec08_polar_radial}

\noindent
The figure shows the radial transport explicitly for $\varphi_0 =
\pi/6$.  Each arrow on the left is the same Cartesian vector
$(1,0)$, drawn at successive radii along the ray.  On the right,
$v^r = \cos\varphi_0$ is constant in $r$, while $v^\varphi$ has
the hyperbolic profile $A/r$ predicted
by~\eqref{eq:radial-vphi}.
...
\input{figures/fig_lec08_geodesic}

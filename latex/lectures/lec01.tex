...
\begin{center}
\begin{tikzpicture}[
  core/.style={rectangle, rounded corners=4pt, draw=none,
    fill=cgblue!15, minimum height=2.2em, text=black,
    font=\sf\small\bfseries, inner sep=6pt},
  block/.style={rectangle, rounded corners=3pt, draw=none,
    fill=isabelline, minimum height=1.8em, minimum width=3.2cm,
    text=black, font=\sf\small, inner sep=5pt},
  arr/.style={-{Stealth[length=5pt]}, thick, spacecadet}
]
  % Hub
  \node[core] (math) at (0,0) {Differential geometry};
  % Spokes (right column, evenly spaced)
  \node[block] (phys)   at (6.0, 2.5) {Physics of gravitation};
  \node[block] (cosmo)  at (6.0, 1.5) {Cosmology};
  \node[block] (qg)     at (6.0, 0.5) {Quantum gravity};
  \node[block] (gauge)  at (6.0,-0.5) {Gauge theory};
  \node[block] (phenom) at (6.0,-1.5) {Phenomenology};
  \node[block] (numrel) at (6.0,-2.5) {Numerical relativity};
  % Arrows
  \foreach \n in {phys,cosmo,qg,gauge,phenom,numrel}
    \draw[arr] (math.east) -- (\n.west);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.1]
  ...
  \begin{scope}[on background layer]
    \fill[black!5, rounded corners=5pt] (-0.5,-0.8) rectangle (6.2,3.5);
  \end{scope}
  % Source mass
  \node[pointmass, minimum size=8pt,
    label={[font=\small]below:{source mass $M$}}] (M) at (5.2,1.2) {};
  % Gravitational field vectors: g(x) = -GM(x-x')/|x-x'|^3
  % Direction toward M, length proportional to 1/r for visual clarity
  \foreach \px/\py in {
    0.5/-0.2, 0.5/0.8, 0.5/1.8, 0.5/2.8,
    1.8/-0.2, 1.8/0.8, 1.8/1.8, 1.8/2.8,
    3.0/-0.2, 3.0/0.5, 3.0/1.8, 3.0/2.8,
    4.2/-0.2, 4.2/0.5, 4.2/2.6, 4.2/3.2,
    5.2/-0.2, 5.2/2.8,
    5.8/0.2, 5.8/2.2} {
    \pgfmathsetmacro{\fdx}{5.2-\px}
    \pgfmathsetmacro{\fdy}{1.2-\py}
    \pgfmathsetmacro{\fr}{max(0.5,sqrt(\fdx*\fdx+\fdy*\fdy))}
    \pgfmathsetmacro{\fa}{min(0.55,0.8/\fr)}
    \draw[fieldline] (\px,\py) -- ++({\fa*\fdx/\fr},{\fa*\fdy/\fr});
  }
  % Field label near a visible vector
  \node[font=\small, anchor=north] at (3.0,-0.55) {$\vb{g}(\vb{x})$};
  ...
  \draw[trajectory, -{Stealth[length=4pt]}] (m) to[out=-25,in=160] (4.0,0.8);
  % Velocity arrow
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_lec01_mass_ratio}
  \caption{Orbits of five test particles launched from the same initial
    conditions ($r_0 = 1$, tangential velocity $v_0 = 0.85\,v_{\mathrm{circ}}$)
    in a Kepler field, with varying mass ratio $\eta = m_g/m_I$.
    If $\eta = 1$ (universality of free fall), all particles follow the
    same orbit; different $\eta$ values produce visibly different
    trajectories.  No such deviation has ever been
    observed.}\label{fig:mass-ratio}
\end{figure}
...
\begin{historical}[Experimental tests of the equivalence principle]
  ...
  \begin{center}
  \begin{tikzpicture}[
    exp/.style={rectangle, rounded corners=2pt, draw=none,
      fill=isabelline, minimum width=2.4cm, minimum height=1.4em,
      font=\small, inner sep=4pt, text=black},
    prec/.style={font=\small\sf, text=black},
    timeline/.style={thick, spacecadet!50}
  ]
    % Timeline
    \draw[timeline] (0,0) -- (12.2,0);
    % Entries
    \node[exp, anchor=south] (N) at (0.8,0.3)  {Newton (1687)};
    \node[prec, anchor=north] at (0.8,-0.2) {$\sim10^{-3}$};

    \node[exp, anchor=south] (E) at (3.8,0.3)
      {E\"otv\"os (1922)};
    \node[prec, anchor=north] at (3.8,-0.2) {$\sim10^{-9}$};

    \node[exp, anchor=south] (LLR) at (6.8,0.3)
      {Lunar laser (1969)};
    \node[prec, anchor=north] at (6.8,-0.2) {$\sim10^{-13}$};

    \node[exp, anchor=south] (MS) at (10.4,0.3)
      {\textsc{microscope} (2022)};
    \node[prec, anchor=north] at (10.4,-0.2) {$\sim10^{-15}$};

    % Dots on timeline
    \foreach \x in {0.8,3.8,6.8,10.4}
      \fill[spacecadet] (\x,0) circle(2pt);
  \end{tikzpicture}
  \end{center}
  \smallskip
  The E\"otv\"os experiment~\cite{Eotvos1922} uses a \emph{torsion balance}:
  two masses of different composition are suspended at opposite ends of a
  bar.  If $m_g/m_I$ differed between materials, the bar would
  rotate.  No such rotation has ever been
  observed~\cite{Roll1964,Adelberger2009}.
  Lunar laser ranging~\cite{Williams2012} exploits the Apollo
  retroreflectors to compare the free-fall accelerations of the Earth and
  Moon toward the Sun.
  The \textsc{microscope} satellite (2016--2018; final results
  published 2022)~\cite{Touboul2022} achieved the most stringent test to
  date, constraining the E\"otv\"os parameter to
  $|\eta| \equiv |(m_g/m_I)_A - (m_g/m_I)_B| < 10^{-15}$.
\end{historical}
...
\begin{tikzpicture}[scale=0.85]
  ...
  \begin{scope}[shift={(-4,0)}]
    ...
    % Charges
    \node[circle, fill=birrenred, inner sep=3pt, font=\footnotesize\sf,
      text=white] (q1) at (-1.2,0) {$+$};
    \node[circle, fill=cgblue, inner sep=3pt, font=\footnotesize\sf,
      text=white] (q2) at (1.2,0) {$-$};
    % Force arrows
    \draw[vecstyle, birrenred] (q1) -- ++(0.6,0)
      node[above,font=\footnotesize]{$\vb{F}$};
    \draw[vecstyle, cgblue] (q2) -- ++(-0.6,0)
      node[above,font=\footnotesize]{$\vb{F}$};
    % Label
...
\begin{tikzpicture}[
  ...
  route/.style={rectangle, rounded corners=3pt, draw=none,
    fill=isabelline, minimum width=2.8cm, minimum height=1.8em,
    font=\small, text=spacecadet, inner sep=4pt, text width=2.6cm,
    align=center},
  outcome/.style={font=\small\sf},
  fail/.style={outcome, text=birrenred},
  ...
  % Four routes
  \node[route] (r0) at (-5.2,-2.8) {(0)~Instantaneous propagation};
  \node[route] (r1) at (-1.8,-2.8) {(1)~Scalar wave eqn for $\phi$};
  \node[route] (r2) at (1.8,-2.8) {(2)~Promote $\phi$ to 4-vector};
  \node[route] (r3) at (5.2,-2.8)
    {(3)~Promote $\phi$ to symmetric tensor};
  ...
  % Outcomes
  \node[fail, below=6pt of r0, text width=2.6cm, align=center]
    {Violates causality\\$\times$};
  \node[fail, below=6pt of r1, text width=2.6cm, align=center]
    {No light bending\\$\times$};
  \node[fail, below=6pt of r2, text width=2.6cm, align=center]
    {Neg-energy waves; perihelion\\$\times$};
  \node[win, below=6pt of r3, text width=2.6cm, align=center]
